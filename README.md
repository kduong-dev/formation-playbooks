...
2. Add `services.yml`, `resources.yml`, `library.yml`, `secrets.yml` (own vault password), `playbook.yml` (set `project`, `formation_root` and `backend_dir`, `import_tasks` `shared/tasks/render-services.yml`, and end with an `import_tasks` of `shared/tasks/render-compose.yml`), `frontend/Dockerfile`, and `domain` / `local_domain` (frontend) and/or `api_domain` / `local_api_domain` in `services.yml`; leave the api ones out if the frontend calls its API by relative path, and the services are routed on the frontend's domains (see remarkable-shelf's)
