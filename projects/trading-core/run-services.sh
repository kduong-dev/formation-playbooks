...
# <subdomain>.<domain> -> host.docker.internal:<port>.
...
write_dynamic_route() {
    ...
    mkdir -p "$DYNAMIC_DIR"
    cat > "$DYNAMIC_DIR/$ROUTE_PREFIX$svc.yml" <<EOF
# Generated by trading-core's run-services.sh — proxy route for $svc.
http:
  routers:
    $ROUTE_PREFIX$svc-proxy:
      rule: "Host(\`$EFFECTIVE_API_DOMAIN\`) && PathPrefix(\`$prefix\`)"
      service: $ROUTE_PREFIX$svc-proxy
      entryPoints: [web]
      # Beats the rendered route for the same host + prefix, which points at
...
remove_dynamic_route() {
    rm -f "$DYNAMIC_DIR/$ROUTE_PREFIX$1.yml"
}
...
watch_proxy_ports() {
    ...
    while sleep 2; do
        for svc in "${proxied[@]}"; do
            ...
            if [[ -f "$f" ]]; then
                port="$(<"$f")"
                last="$(cat "$DYNAMIC_DIR/$ROUTE_PREFIX$svc.last" 2>/dev/null || true)"
                if [[ "$port" != "$last" ]]; then
                    write_dynamic_route "$svc" "$port"
                    echo "$port" > "$DYNAMIC_DIR/$ROUTE_PREFIX$svc.last"
                    echo "[proxy] $svc → host.docker.internal:$port"
                fi
            else
                if [[ -f "$DYNAMIC_DIR/$ROUTE_PREFIX$svc.yml" ]]; then
                    remove_dynamic_route "$svc"
                    rm -f "$DYNAMIC_DIR/$ROUTE_PREFIX$svc.last"
...
teardown_proxy_routes() {
    ...
    # Only remove per-service proxy files — never trading-core.yml (the rendered
    # baseline route table) or other projects' files. Walk SERVICES so we touch
    # known names only.
    for entry in "${SERVICES[@]}"; do
        IFS=':' read -r svc _ _ <<<"$entry"
        rm -f "$DYNAMIC_DIR/$ROUTE_PREFIX$svc.yml" "$DYNAMIC_DIR/$ROUTE_PREFIX$svc.last" 2>/dev/null || true
    done
}
