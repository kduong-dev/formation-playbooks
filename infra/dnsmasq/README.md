...
listens only on the server's LAN interface (`wlp0s20f3`). `run-services.sh up`
refuses to start where that interface doesn't exist (e.g. Docker Desktop):
there dnsmasq would exit and restart in a loop, and local dev uses
`*.localhost`, which needs no DNS.
